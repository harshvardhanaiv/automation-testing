...
test.describe('AIV Login and Create MySQL', () => {
  test('Login, Navigate, and Create MySQL', async ({ page }, testInfo) => {
    ...
    try {
      ...
      await page3.locator('div').filter({ hasText: /^CSV$/ }).locator('div').nth(2).click();
  await page3.getByText('LossFromNetCrime.csv').click();
await page3.getByRole('listitem').filter({ hasText: 'Output Columns' }).locator('span').first().click();
  ...
  await page3.getByRole('textbox', { name: 'Dataset Name' }).fill('ExistingFilecsv');
